...
public class JSONApp {
    ...
    public static JsonObject sessionResult() {
        ...
        String[] subjects = new String[]{"OOP", "English", "Math"};
        int[] marks = new int[]{3, 5, 2};
        boolean[] passes = new boolean[] {true, true, false};
        JsonObject sublist = null;
        JsonObject[] mainList = new JsonObject[3];
        for (int i = 0; i < 3; i++){
            JsonPair subject = new JsonPair("course", new JsonString(subjects[i]));
            JsonPair mark = new JsonPair("mark", new JsonNumber(marks[i]));
            JsonPair pass = new JsonPair("passed", new JsonBoolean(passes[i]));
